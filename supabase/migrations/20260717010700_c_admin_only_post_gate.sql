-- EV-C: During soak, restrict post_import_batch_v2 to system_admin when flag is on.
-- Import officers gain access after closure expands the gate (set import_workflow_v2_officer_enabled).

INSERT INTO public.system_settings (key, value, category)
SELECT 'import_workflow_v2_officer_enabled', 'false', 'import'
WHERE NOT EXISTS (
  SELECT 1 FROM public.system_settings WHERE key = 'import_workflow_v2_officer_enabled'
);
...
BEGIN
  ...
  SELECT value INTO v_officer_flag
  FROM public.system_settings
  WHERE key = 'import_workflow_v2_officer_enabled'
  LIMIT 1;

  IF NOT public.current_user_is_system_admin()
     AND coalesce(v_officer_flag, 'false') <> 'true' THEN
    RAISE EXCEPTION 'EV-C denied: admin-only soak; import_officer posting not enabled'
      USING ERRCODE = '42501';
  END IF;

  IF NOT public.current_user_is_system_admin()
     AND public.current_user_role() NOT IN (
       'import_officer', 'operations_manager', 'company_manager', 'station_manager'
     ) THEN
    RAISE EXCEPTION 'EV-C denied: role cannot post import v2' USING ERRCODE = '42501';
  END IF;
