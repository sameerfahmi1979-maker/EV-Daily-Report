-- EV-E Migration 8: Remove redundant per-row station-access re-check.
--
-- Every report_* RPC already calls report_assert_access(p_station_id) once, up front:
--   - Global roles (admin/ops/company manager) may pass p_station_id = NULL (any station).
--   - Station-scoped roles MUST pass a non-null p_station_id, and report_assert_access
--     already verifies they have access to exactly that station before the query runs.
-- The query's own "(p_station_id IS NULL OR cs.station_id = p_station_id)" filter then
-- guarantees every returned row belongs either to the single pre-authorized station, or
-- (for global roles only, where p_station_id may be NULL) is unrestricted by design.
--
-- The additional per-row "OR current_user_has_station_access(cs.station_id)" was
-- therefore fully redundant, and expensive: it re-runs a subquery against
-- user_station_access/user_profiles for every single row. At production scale (50k+
-- rows in a bounded 230-day window) this alone pushed several RPCs to the platform's ~8s
-- timeout. Removing it preserves identical security semantics while restoring
-- performance headroom well under 400-day windows.
