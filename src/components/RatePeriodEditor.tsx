...
export default function RatePeriodEditor({ rateStructureId, rateStructureName, onClose }: RatePeriodEditorProps) {
  ...
  function validatePeriodForm(): boolean {
    ...
    // EV-B: demand charge forced to zero (field retained for schema soak)
  ...
  const coverage = validateDayCoverage(
    periods.filter((p) => p.season === 'all' || p.season === 'summer')
  );
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            {periods
              ...
              .flatMap((period) => {
                ...
                return intervals.map((iv, idx) => {
                  const style = intervalToStyle(iv);
                  return (
                    <div
                      key={`${period.id}-${idx}`}
        ...
        <div className="mt-2 text-xs">
          {coverage.complete ? (
            <span className="text-green-700 font-medium">Complete 24-hour coverage</span>
          ) : (
            <span className="text-red-700 font-medium">
              Incomplete coverage — gaps: {coverage.uncoveredMinutes} min
              {coverage.overlaps.length > 0 ? `, overlaps: ${coverage.overlaps.length} min` : ''}
            </span>
          )}
        </div>
          ...
          <div className="mt-4 relative h-12 bg-gray-50 rounded-lg border border-gray-300">
            ...
            <div className="absolute inset-0 top-6">
              {periods
                ...
                .flatMap((period) => {
                  const color = getPeriodColor(period.period_name);
                  return expandPeriodToDisplayIntervals(period.start_time, period.end_time).map(
                    (iv, idx) => {
                      const style = intervalToStyle(iv);
                      return (
                        <div
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleSubmitPeriod} className="space-y-4">
                ...
                <div>
                  ...
                  />
                  {errors.energy_rate_per_kwh && (
                    <p className="text-red-600 text-sm mt-1">{errors.energy_rate_per_kwh}</p>
                  )}
                  <p className="text-xs text-gray-500 mt-1">Demand Charge retired (always 0) for Jordan billing.</p>
                </div>
