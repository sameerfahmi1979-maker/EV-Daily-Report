...
import LegacyReportBanner from './LegacyReportBanner';
import { useAuth } from '../contexts/AuthContext';
import { normalizeRole } from '../lib/rbac';
import {
  Clock, Filter, Search, Eye, ChevronDown, ChevronLeft, ChevronRight,
...
import {
  getShiftsPaginated, updateHandoverStatus, uploadDepositSlip, deleteShift,
  recalculateShiftTotals, recalculateAllShiftTotals,
  getShiftSessionsWithBilling,
  Shift, SHIFT_TYPES
} from '../lib/shiftService';
import { normalizeSessionsBillingEmbed } from '../lib/billingService';
import { generateShiftSessionReportPDF, generateMoneyHandoverLetterPDF, HandoverPaymentSummary } from '../lib/pdfReportService';
import { getStations } from '../lib/stationService';
import { supabase } from '../lib/supabase';
import { Database } from '../lib/database.types';
import PaymentHandoverPanel from './PaymentHandoverPanel';
import { listHandoversForShift } from '../lib/paymentHandoverService';
...
// Workflow order: pending -> printed -> handed_over -> deposited.
const HANDOVER_STATUS_ORDER = ['pending', 'printed', 'handed_over', 'deposited'] as const;
...
export default function ShiftManagement() {
  ...
  // Debounce search input -> searchQuery (also resets to page 1)
  useEffect(() => {
    const t = setTimeout(() => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [shiftsResult, stationsData, operatorsResult] = await Promise.all([
        getShiftsPaginated({
      ...
      setTotalCount(shiftsResult.totalCount);
      setTotalRevenue(shiftsResult.totalRevenue);
      setTotalKwh(shiftsResult.totalKwh);
      setPendingCount(shiftsResult.pendingCount);
  ...
  }, [filterStation, filterOperator, filterStatus, filterDateFrom, filterDateTo, searchQuery, page, pageSize]);
  ...
  async function loadShiftSessions(shiftId: string) {
    ...
    try {
      ...
      setShiftSessions(normalizeSessionsBillingEmbed(data));
    } catch (err) {
  ...
  /** The current (non-cancelled/rejected) cash handover for a shift, if any, for the Cash/CliQ/Card breakdown on printed PDFs. */
  async function getActiveHandoverSummary(shiftId: string): Promise<HandoverPaymentSummary | null> {
    try {
      const handovers = await listHandoversForShift(shiftId);
      const active = handovers.find((h: any) => h.status !== 'cancelled' && h.status !== 'rejected');
      if (!active) return null;
      return {
        cash_total: active.cash_total,
        cliq_total: active.cliq_total,
        card_total: active.card_total,
        shortage_amount: active.shortage_amount,
        surplus_amount: active.surplus_amount,
        discrepancy_reason: active.discrepancy_reason,
        status: active.status,
      };
    } catch (err) {
      console.error('Failed to load handover for PDF:', err);
      return null;
    }
  }
  ...
  async function handlePrintSessionReport(shift: Shift) {
    ...
    try {
      const [sessions, handover] = await Promise.all([
        getShiftSessionsWithBilling(shift.id),
        getActiveHandoverSummary(shift.id),
      ]);
      await generateShiftSessionReportPDF(shift as any, sessions, handover);
    } catch (err) {
  ...
  async function handlePrintHandoverLetter(shift: Shift) {
    ...
    try {
      ...
      await generateMoneyHandoverLetterPDF(shift as any, sessions.length, handover);
    } catch (err) {
                        ...
                        <div className="flex items-center justify-center gap-1">
                          ...
                          {isSystemAdmin && (
                            <button
                              ...
                              title="Permanently delete this shift and all related records (System Administrator only)"
                            >
                              <Trash2 size={16} />
                            </button>
                          )}
      ...
      {!loading && totalCount > 0 && (
        <div className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-4">
          <div className="text-sm text-gray-600">
            Showing {(page - 1) * pageSize + 1} - {Math.min(page * pageSize, totalCount)} of {totalCount} shifts
          </div>
          {totalPages > 1 && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl mx-4 mb-10" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-6">
              ...
              <PaymentHandoverPanel
                ...
                shiftTotal={selectedShift.total_amount_jod}
              />
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  {HANDOVER_STATUS_ORDER.map((status, idx) => {
                    const config = STATUS_CONFIG[status];
                    const StatusIcon = config.icon;
                    const isCurrent = selectedShift.handover_status === status;
                    const isPast = HANDOVER_STATUS_ORDER.indexOf(selectedShift.handover_status as any) > idx;
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={() => setDeletingShiftId(null)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <ul className="text-sm text-gray-700 bg-red-50 rounded-lg p-3 mb-4 space-y-1">
              <li>• All charging sessions from this shift</li>
