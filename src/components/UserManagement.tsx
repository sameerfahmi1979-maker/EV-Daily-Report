...
import {
  ...
  Trash2, KeyRound, Eye, EyeOff, AlertTriangle,
} from 'lucide-react';
import {
  getAllUsers, updateUserProfile, UserProfile,
  ROLE_LABELS, ROLE_COLORS, UserRole, PERMISSIONS, hasPermission,
} from '../lib/userService';
import { adminDeleteUser, adminSetUserPassword } from '../lib/userAdminService';
...
export default function UserManagement() {
  ...
  const isSystemAdmin = currentProfile?.role === 'system_admin' || currentProfile?.role === 'global_admin';
  ...
  async function handleSavePassword() {
    ...
    setPwSaving(true);
    try {
      await adminSetUserPassword(passwordUser.id, newPassword, forceChange);
      setPwSuccess(true);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ==================== DELETE USER CONFIRM ==================== */}
      {deletingUser && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={() => setDeletingUser(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                  <AlertTriangle size={18} className="text-red-600" /> Delete User
                </h3>
                <button onClick={() => setDeletingUser(null)} className="p-2 hover:bg-gray-100 rounded-lg">
                  <X size={20} className="text-gray-500" />
                </button>
              </div>
            </div>
            <div className="p-6 space-y-4">
              ...
              <p className="text-sm text-gray-700">
                This will permanently delete <strong>{deletingUser.full_name || deletingUser.email}</strong>{' '}
                (<span className="text-gray-500">{deletingUser.email}</span>). They will never be able to log
                in again with this account. This action <strong>cannot be undone</strong>.
              </p>
              <div className="flex gap-3 pt-2">
                <button
                ...
                >
                  {deleteSaving ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />}
      ...
      {/* ==================== SET PASSWORD MODAL ==================== */}
      {passwordUser && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={() => setPasswordUser(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                  <KeyRound size={18} className="text-amber-600" /> Set Password
                </h3>
                <button onClick={() => setPasswordUser(null)} className="p-2 hover:bg-gray-100 rounded-lg">
                  <X size={20} className="text-gray-500" />
                </button>
              </div>
              <p className="text-sm text-gray-500 mt-1">{passwordUser.email} — {passwordUser.full_name}</p>
            </div>
            ...
            {pwSuccess ? (
              <div className="p-6 text-center space-y-3">
                ...
                <p className="text-sm text-gray-500">
                  {forceChange
                    ? 'The user will be required to set a new password the next time they log in.'
              ...
              <div className="p-6 space-y-4">
                {pwError && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex items-center gap-2 text-sm text-red-800">
                    <AlertCircle size={16} className="flex-shrink-0" /> {pwError}
                  </div>
                )}
                ...
                <label className="flex items-center gap-3 py-2 px-3 bg-gray-50 rounded-lg cursor-pointer">
                  <input
                    type="checkbox"
                    checked={forceChange}
                  ...
                  />
                  <div>
                    <p className="text-sm font-medium text-gray-700">Require password change on next login</p>
                    <p className="text-xs text-gray-500">
                      Treats this as a temporary password — the user must set their own before using the dashboard.
                    </p>
                  </div>
                </label>
