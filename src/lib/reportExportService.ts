...
// =====================================================================
// COLLECTION REPORT (Cash / CliQ / Card collected for a period)
// =====================================================================

export const collectionReportColumns: ColumnDef[] = [
  { header: 'Date', key: 'date', width: 14 },
  { header: 'Cash (JOD)', key: 'cash', width: 14, format: (v: number) => v.toFixed(3) },
  { header: 'CliQ (JOD)', key: 'cliq', width: 14, format: (v: number) => v.toFixed(3) },
  { header: 'Card (JOD)', key: 'card', width: 14, format: (v: number) => v.toFixed(3) },
  { header: 'Total (JOD)', key: 'total', width: 14, format: (v: number) => v.toFixed(3) },
];
...
/** Collection Report PDF: KPI strip, side-by-side charts, a short written analysis, then the daily breakdown table. */
export async function exportCollectionReportPDF(
  summary: CollectionReportSummary,
  dailyRows: CollectionDailyRow[],
  filters: any,
  filename: string,
  charts: Array<{ title: string; image: string }>,
  ...
  const doc = new jsPDF({ orientation: 'landscape', compress: true });
  let y = await addBrandedPdfHeader(doc, 'Collection Report — Cash / CliQ / Card', periodStr(filters));
  y = addFilterSummary(doc, buildFilterSummary('Collection Report', filters), y);
  y = addSummaryStrip(
    doc,
    `Total Collected: ${formatJOD(summary.total)} JOD   |   Cash: ${formatJOD(summary.cash)}   |   CliQ: ${formatJOD(summary.cliq)}   |   Card: ${formatJOD(summary.card)}   |   Handovers: ${summary.count}`,
    y
  );
  ...
  if (charts.length) {
    ...
    for (const chart of charts) {
      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.text(chart.title, chartX, chartY);
      doc.addImage(chart.image, 'PNG', chartX, chartY + 3, 125, 62);
      chartX += 133;
    }
    y = chartY + 70;
  }
  ...
  const maxTextWidth = doc.internal.pageSize.width - 28;
  ...
  autoTable(doc, {
    startY: y,
    head: [collectionReportColumns.map((c) => c.header)],
    body: dailyRows.map((r) => collectionReportColumns.map((c) => {
      const v = (r as any)[c.key];
      return c.format ? c.format(v) : v ?? '';
    })),
    foot: [[
      'TOTAL',
      summary.cash.toFixed(3),
      summary.cliq.toFixed(3),
      summary.card.toFixed(3),
