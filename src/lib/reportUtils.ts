...
function withStagingFilename(filename: string): string {
  if (import.meta.env.VITE_APP_ENV !== 'staging') return filename;
  if (filename.includes('_STAGING')) return filename;
  const dot = filename.lastIndexOf('.');
  if (dot <= 0) return `${filename}_STAGING`;
  return `${filename.slice(0, dot)}_STAGING${filename.slice(dot)}`;
}

export function addPdfFooter(doc: jsPDF, footerText?: string) {
  if (import.meta.env.VITE_APP_ENV === 'staging') {
    const label = 'STAGING — NOT FOR FINANCIAL USE';
    footerText = footerText ? `${footerText} | ${label}` : label;
  }
...
export function downloadBlob(blob: Blob, filename: string) {
  ...
  a.href = url;
  a.download = withStagingFilename(filename);
  a.click();
...
export function downloadPdf(doc: jsPDF, filename: string) {
  ...
  downloadBlob(blob, withStagingFilename(filename));
}
...
export function exportToExcel(
  ...
  XLSX.writeFile(wb, withStagingFilename(filename));
}
