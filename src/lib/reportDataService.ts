...
import { normalizeBillingEmbed } from './billingService';
...
function computeTotals(sessions: any[]): Totals {
  return {
    ...
    revenue: sessions.reduce((s, r) => {
      const b = normalizeBillingEmbed(r.billing_calculations)[0];
...
function mapSession(s: any): TransactionRow {
  const billing = normalizeBillingEmbed(s.billing_calculations);
  return {
    id: s.id,
    ...
    cost_jod: billing[0]?.total_amount ? parseFloat(billing[0].total_amount) : null,
    status: billing.length ? 'Calculated' : 'Pending',
...
export async function fetchPendingBilling(filters: BaseFilter) {
  ...
  const unbilled = sessions.filter(
    (s: any) => normalizeBillingEmbed(s.billing_calculations).length === 0
  );
