...
// "Pending Deposits" = cash already handed over to accounts, awaiting bank deposit.
export async function getPendingDeposits(): Promise<PendingShift[]> {
  const { data, error } = await supabase
    ...
    .eq('handover_status', 'handed_over')
  ...
  return (data || []).map((sh: any) => {
    ...
    return {
      ...
      handover_status: sh.handover_status || 'handed_over',
...
// "Awaiting Handover to Accounts" = cash still with the operator/shift (report
// printed at most), not yet physically handed over to the accounts team.
...
export async function getFinancialSummary(startDate: string, endDate: string) {
  ...
  // Workflow order: pending -> printed -> handed_over -> deposited ("deposited" is the final, completed stage).
  const pendingCount = shifts.filter((sh: any) => sh.handover_status === 'pending' || sh.handover_status === 'printed').length;
  const awaitingDepositCount = shifts.filter((sh: any) => sh.handover_status === 'handed_over').length;
  const depositedCount = shifts.filter((sh: any) => sh.handover_status === 'deposited').length;

  // `handedOverCount` is kept for backward compatibility with the "Completed" KPI in
  // AccountantDashboard; it now represents the final/completed stage (deposited).
  return { totalRevenue, totalKwh, totalShifts: shifts.length, pendingCount, depositedCount, awaitingDepositCount, handedOverCount: depositedCount };
}
