...
type Json = Database['public']['Tables']['billing_calculations']['Row']['breakdown'];

/**
 * Since billing_calculations.session_id became UNIQUE (Phase A1, one billing
 * row per session), PostgREST infers a to-one relationship for any nested
 * `charging_sessions -> billing_calculations(...)` embed and returns a single
 * object instead of an array. All call sites across this codebase were
 * written against the old to-many shape (`.length`, `[0]`, `.filter(...)`),
 * so every row silently looked "Pending" even after a successful calculation.
 * Apply this immediately after any query that embeds billing_calculations to
 * restore the array shape every consumer expects — do not remove without
 * auditing every `billing_calculations` consumer in the codebase first.
 */
export function normalizeBillingEmbed<T>(raw: T | T[] | null | undefined): T[] {
  if (!raw) return [];
  return Array.isArray(raw) ? raw : [raw];
}

export function normalizeSessionsBillingEmbed<T extends { billing_calculations?: unknown }>(
  rows: T[] | null | undefined
): T[] {
  return (rows || []).map((row) => ({
    ...row,
    billing_calculations: normalizeBillingEmbed(row.billing_calculations as any),
  }));
}
...
export async function calculateSessionBilling(
  ...
  // EV-B: Demand Charge retired from active Jordan billing (always 0)
  const periodCharges: PeriodCharge[] = segmentsWithEnergy.map(segment => {
    const energyCharge = Number(((segment.energyKwh || 0) * segment.ratePerKwh).toFixed(3));
...
export async function saveBillingCalculation(
  ...
  // EV-A1: persist via transactional RPC so UNIQUE(session_id) cannot be violated
  // by client-side delete+insert races. Tariff algorithm unchanged.
  const breakdownItems = breakdown.periodCharges.map(pc => ({
    period_name: pc.periodName,
    start_time: pc.startTime.toISOString(),
    end_time: pc.endTime.toISOString(),
    duration_minutes: pc.duration,
    energy_kwh: Number(pc.energy.toFixed(3)),
    rate_per_kwh: Number(pc.ratePerKwh.toFixed(3)),
    demand_kw: Number(pc.demand.toFixed(3)),
    demand_charge: Number(pc.demandCharge.toFixed(3)),
    energy_charge: Number(pc.energyCharge.toFixed(3)),
    line_total: Number(pc.lineTotal.toFixed(3)),
  }));

  const { data: rpcResult, error: rpcError } = await supabase.rpc('replace_session_billing', {
    p_session_id: sessionId,
    p_rate_structure_id: rateStructureId,
    p_subtotal: Number(breakdown.subtotal.toFixed(3)),
    p_taxes: Number(breakdown.taxes.toFixed(3)),
    p_fees: Number(breakdown.fixedCharges.toFixed(3)),
    p_total_amount: Number(breakdown.total.toFixed(3)),
    p_currency: 'JOD',
    p_breakdown: breakdown as unknown as Json,
    p_breakdown_items: breakdownItems as unknown as Json,
  });

  if (rpcError) throw rpcError;

  const billingId =
    rpcResult && typeof rpcResult === 'object' && 'billing_id' in rpcResult
      ? String((rpcResult as { billing_id: string }).billing_id)
      : null;

  if (!billingId) {
    throw new Error('replace_session_billing did not return billing_id');
  }
...
export async function recalculateSession(
  sessionId: string,
  reason: string = 'manual_recalculate'
): Promise<BillingCalculation> {
  // EV-B: authoritative persistence via calculate_session_billing_v2
  const { data: rpcResult, error: rpcError } = await supabase.rpc(
    'recalculate_session_billing_v2',
    { p_session_id: sessionId, p_reason: reason }
  );
  if (rpcError) {
    // Fallback preview path only if v2 not deployed yet
    console.warn('recalculate_session_billing_v2 unavailable, using client path:', rpcError.message);
  ...
  const billingId =
    rpcResult && typeof rpcResult === 'object' && 'billing_id' in rpcResult
      ? String((rpcResult as { billing_id: string }).billing_id)
      : null;
  if (!billingId) throw new Error('recalculate_session_billing_v2 did not return billing_id');
  const { data: billing, error: billingError } = await supabase
    .from('billing_calculations')
    .select('*')
    .eq('id', billingId)
...
export async function getSessionsWithBillingFiltered(filters: SessionFilters = {}): Promise<PaginatedSessionsResult> {
  ...
  // EV-F hotfix: this used to query charging_sessions directly, which paid
  // the sessions_select_scoped RLS cost per row (current_user_has_station_access
  // evaluated as a security-barrier qualifier). That was fine at low row
  // counts but times out well past ~80k sessions. get_sessions_with_billing_filtered
  // is a SECURITY DEFINER RPC that resolves station access once instead of
  // per row and replicates the exact same station scoping explicitly.
  const { data, error } = await supabase.rpc('get_sessions_with_billing_filtered', {
  ...
  const result = data as { sessions: any[]; total_count: number };
  const sessions = normalizeSessionsBillingEmbed(result?.sessions);
