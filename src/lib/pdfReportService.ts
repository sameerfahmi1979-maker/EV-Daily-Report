...
// ========================================================
/** Cash/CliQ/Card breakdown collected at handover (Manual Shift Cash Settlement) */
export interface HandoverPaymentSummary {
  cash_total?: number | null;
  cliq_total?: number | null;
...
export async function generateShiftSessionReportPDF(
  ...
  handover?: HandoverPaymentSummary | null
): Promise<void> {
  const s = await getSettings();
  const doc = new jsPDF();
...
function addPaymentBreakdownSection(doc: jsPDF, startY: number, handover?: HandoverPaymentSummary | null): number {
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(30, 58, 138);
  doc.text('PAYMENT COLLECTED FROM OPERATOR', 14, y);
  doc.setTextColor(0);
  y += 5;
  ...
  if (!handover) {
    ...
    doc.text('Not yet settled — no cash handover has been recorded for this shift.', 14, y);
    doc.setTextColor(0);
    return y + 4;
  }

  const cash = Number(handover.cash_total || 0);
  const cliq = Number(handover.cliq_total || 0);
  const card = Number(handover.card_total || 0);
  const total = cash + cliq + card;
  const shortage = Number(handover.shortage_amount || 0);
  const surplus = Number(handover.surplus_amount || 0);

  autoTable(doc, {
    startY: y,
    head: [['Cash', 'CliQ', 'Card', 'Total Collected']],
    body: [[formatJOD(cash), formatJOD(cliq), formatJOD(card), formatJOD(total)]],
  ...
  if (shortage > 0.0005 || surplus > 0.0005) {
    const isShortage = shortage > 0.0005;
    // Shortage (negative — money missing) is red; surplus (positive — extra money) is green.
    const fill: [number, number, number] = isShortage ? [254, 242, 242] : [236, 253, 245];
    const border: [number, number, number] = isShortage ? [220, 38, 38] : [5, 150, 105];
    const textColor: [number, number, number] = isShortage ? [185, 28, 28] : [6, 95, 70];

    doc.setFillColor(...fill);
    doc.setDrawColor(...border);
    const boxHeight = isShortage && handover.discrepancy_reason ? 14 : 8;
    doc.roundedRect(14, y, contentW, boxHeight, 2, 2, 'FD');
    doc.setFontSize(9);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...textColor);
    const label = isShortage
      ? `SHORTAGE (MISS): ${formatJOD(shortage)} — to be deducted from operator's salary`
      : `SURPLUS: ${formatJOD(surplus)} — extra amount collected`;
