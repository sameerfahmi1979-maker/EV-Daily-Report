...
import { formatJOD, normalizeSessionsBillingEmbed } from './billingService';
import { getAllSettings } from './settingsService';

// Base URL the invoice verification QR code points to. Defaults to the
// production app URL; override with VITE_PUBLIC_APP_URL for other
// environments (staging/local) so QR codes there don't point at production.
const PUBLIC_APP_URL =
  (import.meta.env.VITE_PUBLIC_APP_URL as string | undefined) || 'https://crm.energy-stream.net';

/** Public URL that opens the invoice verification page for a given token. */
export function buildInvoiceVerificationUrl(token: string): string {
  return `${PUBLIC_APP_URL.replace(/\/+$/, '')}/?verify=${token}`;
}
...
export async function getInvoiceData(sessionId: string): Promise<InvoiceData | null> {
  ...
  return {
    ...
    verificationToken: billing.verification_token,
    session: {
...
export async function generateInvoicePDF(invoice: InvoiceData) {
  ...
  // QR code: scanning it opens the public invoice verification page.
  if (invoice.verificationToken) {
    try {
      const verifyUrl = buildInvoiceVerificationUrl(invoice.verificationToken);
      const qrDataUrl = await QRCode.toDataURL(verifyUrl, { width: 240, margin: 1 });
      const qrSize = 26;
      const qrX = 297 - 20 - qrSize; // 297mm = landscape A4 width, 20mm right margin
      const qrY = 6;
      ...
      console.warn('Failed to generate invoice verification QR code:', e);
    }
  }
...
export async function exportSessionsToExcel(startDate: Date, endDate: Date, stationId?: string, includeCharts: boolean = true, startTime?: string, endTime?: string): Promise<number> {
  ...
  const sessions = normalizeSessionsBillingEmbed(rawSessions);
...
export async function exportSessionsToPDF(startDate: Date, endDate: Date, stationId?: string, includeCharts: boolean = true, startTime?: string, endTime?: string, cardNumber?: string): Promise<number> {
  ...
  const sessions = normalizeSessionsBillingEmbed(await fetchAllRows(
    'charging_sessions',
...
export async function exportBillingToPDF(startDate: Date, endDate: Date, stationId?: string, includeCharts: boolean = true, startTime?: string, endTime?: string, cardNumber?: string): Promise<number> {
  ...
  const allSessions = normalizeSessionsBillingEmbed(await fetchAllRows(
    'charging_sessions',
