...
export interface CreateImportBatchOptions {
  fileHash?: string;
...
export async function createImportBatch(
  ...
  totalRecords: number,
  userId: string,
  options?: CreateImportBatchOptions
): Promise<string> {
  ...
  if (options?.fileHash) row.file_hash = options.fileHash;
  if (options?.fileSizeBytes != null) row.file_size_bytes = options.fileSizeBytes;
  if (options?.normalizedFilename) row.normalized_filename = options.normalizedFilename;
  if (options?.sheetName) row.sheet_name = options.sheetName;
  if (options?.parserVersion) row.parser_version = options.parserVersion;
  if (options?.selectedOperatorId) row.selected_operator_id = options.selectedOperatorId;
  if (options?.detectedCardId !== undefined) row.detected_card_id = options.detectedCardId;
  if (options?.detectedOperatorName !== undefined) {
    row.detected_operator_name = options.detectedOperatorName;
  }
  if (options?.operatorMatchStatus) row.operator_match_status = options.operatorMatchStatus;
  if (options?.stationId) row.station_id = options.stationId;
  if (options?.validationSummary) row.validation_summary = options.validationSummary;
  if (options?.workflowVersion) row.workflow_version = options.workflowVersion;

  const { data, error } = await supabase
    .from('import_batches')
    .insert([row as ImportBatchInsert])
    .select()
    .single();
...
export function parseSessionDateTime(
  dateTimeStr: string
): { date: string; time: string; timestamp: string } | null {
...
function prepareSessionRow(
  ...
  // EV-B: authoritative billing sets calculated_cost; do not use 0.150 fallback
  const calculatedCost = session.cost ?? 0;

  const row: ChargingSessionInsert & Record<string, unknown> = {
  ...
  if (extras?.operatorId) row.operator_id = extras.operatorId;
  if (extras?.sourceRowNumber != null) row.source_row_number = extras.sourceRowNumber;
  if (extras?.sourceFileHash) {
    row.source_file_hash = extras.sourceFileHash;
    row.source_transaction_id = session.transactionId;
  }

  return row as ChargingSessionInsert;
}

// =============================================
// OPTIMIZED: Pre-check duplicates in ONE query
// =============================================
export async function previewDuplicateTransactionIds(transactionIds: string[]): Promise<Set<string>> {
  return checkDuplicateTransactionIds(transactionIds);
}
...
export async function processBatch(
  ...
  // ── STEP 5: Authoritative server billing (EV-B v2 when enabled) ──
  if (successCount > 0 && stationId && !cancelToken?.cancelled) {
    try {
      const { data: flagRow } = await supabase
        .from('system_settings')
        .select('value')
        .eq('key', 'billing_engine_v2_enabled')
        .maybeSingle();
      const useV2 = (flagRow?.value ?? 'true') === 'true';

      if (useV2) {
        console.log(`Calling calculate_batch_billing_v2 for batch ${batchId}...`);
        const { data: billingResult, error: billingError } = await supabase.rpc(
          'calculate_batch_billing_v2',
          { p_batch_id: batchId, p_station_id: stationId }
        );
        if (billingError) {
          console.error('Batch billing v2 failed:', billingError.message);
          errors.push({
            row: 0,
            session: sessions[0],
            errors: [`Billing engine v2 failed: ${billingError.message}`],
          });
        } else {
          console.log('Batch billing v2 completed:', billingResult);
        }
      } else {
        console.log(`Calling legacy calculate_batch_billing for batch ${batchId}...`);
        const { data: billingResult, error: billingError } = await supabase.rpc(
      ...
      console.error('Batch billing failed:', billingErr);
    }
  }
...
/**
 * Permanently, irreversibly deletes an import batch and every descendant row
 * across the whole schema (sessions, billing, breakdown, payment
 * allocations, cash handovers + adjustments/events, historical correction
 * queue/archive, payment classification queue, engine metadata repair log).
 *
 ...
 * undo. See admin_force_delete_import_batch in the database.
 */
export async function deleteImportBatchCascade(batchId: string): Promise<{
  ...
  const { data, error } = await supabase.rpc('admin_force_delete_import_batch', {
    p_batch_id: batchId,
  });
  if (error) throw error;
  ...
  return {
    ...
    shifts_deleted: result?.shifts_deleted ?? 0,
  };
}

export async function isImportWorkflowV2Enabled(): Promise<boolean> {
  const { data } = await supabase
    .from('system_settings')
    .select('value')
    .eq('key', 'import_workflow_v2_enabled')
    .maybeSingle();
  return (data?.value ?? 'false') === 'true';
}

export async function findOperatorByNormalizedCard(
  normalizedCard: string
): Promise<{ id: string; name: string; card_number: string; card_number_normalized?: string | null; status?: string | null } | null> {
  const { data, error } = await supabase
    .from('operators')
    .select('id, name, card_number, card_number_normalized, status')
    .eq('card_number_normalized', normalizedCard)
    .eq('status', 'active')
    .limit(1)
    .maybeSingle();
  ...
  if (error) {
    ...
    const { data: fallback } = await supabase
      .from('operators')
      .select('id, name, card_number, status')
...
export async function findPostedBatchByFileHash(fileHash: string): Promise<{ id: string; filename: string; status: string | null } | null> {
  const { data } = await supabase
    ...
    .in('status', ['posted', 'completed', 'completed_with_errors', 'billing_failed', 'partially_posted'])
...
export async function postImportBatchV2(params: {
  batchId: string;
  stationId: string;
  operatorId: string;
  fileHash: string;
  sessions: ParsedSession[];
  allowFilenameWarning?: boolean;
  allowConflictOverride?: boolean;
}): Promise<Record<string, unknown>> {
  const { sessionsToPostPayload } = await import('./importIntegrity');
  const payload = sessionsToPostPayload(params.sessions, parseDateTimeString);
  const { data, error } = await supabase.rpc('post_import_batch_v2', {
    p_batch_id: params.batchId,
    p_station_id: params.stationId,
    p_operator_id: params.operatorId,
    p_file_hash: params.fileHash,
    p_sessions: payload,
    p_allow_filename_warning: params.allowFilenameWarning ?? false,
    p_allow_conflict_override: params.allowConflictOverride ?? false,
  });
  if (error) throw error;
